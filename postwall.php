<?php session_start();
	...
	<p><a href="index.php?page=userhomepage">Back to my homepage</a></p>
	<?php
	}
    ?>
<?php
	$user=$_SESSION['admin'];
	$postwall_sql="SELECT pid, pname, post_time, title, content, multimedia, location FROM POST WHERE pname='{$user}'
		union 
		SELECT pid, pname, post_time, title, content, multimedia, location FROM POST inner join friendship on pname = friends WHERE authorization=0 or users = '{$user}'
...
?>
	<?php
		...
		?>
		<h1>Sorry, there is no new post right now.</h1>
		<?php
	}
?>